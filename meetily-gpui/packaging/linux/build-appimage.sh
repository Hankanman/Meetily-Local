#!/usr/bin/env bash
# Packages a staged meetily-gpui dist dir into a Parley-<version>-x86_64.AppImage
...
#
# Args:
#   $1  DIST_DIR   staged dir: meetily-gpui binary + .so files + llama-helper
...
if [[ ! -x "$DIST_DIR/meetily-gpui" ]]; then
    echo "error: $DIST_DIR/meetily-gpui not found or not executable" >&2
    exit 1
fi

# ----- fetch linuxdeploy (cached) -----
TOOLS_CACHE_DIR="${MEETILY_APPIMAGE_TOOLS_DIR:-$HOME/.cache/meetily-appimage-tools}"
...
chmod +x "$APPDIR/usr/bin/meetily-gpui" "$APPDIR/usr/bin/llama-helper" 2>/dev/null || true
...
cp "$ICON_SRC" "$APPDIR/usr/share/icons/hicolor/512x512/apps/com.meetily.ai.png"
