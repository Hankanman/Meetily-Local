    ...
    SettingPage::new("Integrations")
        ...
        .group(
            SettingGroup::new()
                ...
                .description(
                    "meetily-mcp is a separate binary an AI client (Claude Desktop, Claude Code) \
...
fn info_item(view: &Entity<SettingsView>) -> SettingItem {
    ...
    SettingItem::render(move |_options, _window, cx| {
        ...
        let bin_display = info
            .binary_path
            .clone()
            .unwrap_or_else(|| "/absolute/path/to/meetily-mcp".to_string());
...
/// The `claude mcp add` one-liner for registering `meetily-mcp` with Claude
...
fn claude_code_snippet(binary_path: &str, db_path: &str, db_is_default: bool) -> String {
    if db_is_default {
        format!("claude mcp add meetily -- {}", binary_path)
    } else {
        format!("claude mcp add meetily -- {} --db {}", binary_path, db_path)
    }
}
...
mod tests {
    ...
    fn claude_code_snippet_omits_db_flag_for_the_default_path() {
        let snippet = claude_code_snippet("/bin/meetily-mcp", "/data/meeting_minutes.sqlite", true);
        assert_eq!(snippet, "claude mcp add meetily -- /bin/meetily-mcp");
    }

    #[test]
    fn claude_code_snippet_includes_db_flag_for_a_non_default_path() {
        let snippet = claude_code_snippet("/bin/meetily-mcp", "/custom/db.sqlite", false);
        assert_eq!(
            snippet,
            "claude mcp add meetily -- /bin/meetily-mcp --db /custom/db.sqlite"
        );
    }

    #[test]
    fn json_snippet_omits_args_for_the_default_path() {
        let snippet = json_snippet("/bin/meetily-mcp", "/data/meeting_minutes.sqlite", true);
        assert!(snippet.contains("\"command\": \"/bin/meetily-mcp\""));
        assert!(!snippet.contains("args"));
    }

    #[test]
    fn json_snippet_includes_args_for_a_non_default_path() {
        let snippet = json_snippet("/bin/meetily-mcp", "/custom/db.sqlite", false);
