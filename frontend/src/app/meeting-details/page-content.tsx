...
export default function PageContent({
  ...
  //
  // Guarded by a ref because (1) `summaryGeneration` and `onAutoGenerateComplete`
  // are fresh references on every render and (2) `processSummary` immediately
  // calls `setSummaryStatus("processing")` which re-renders this component —
  // so any dep that changes per-render would re-fire the effect mid-flight,
  // spawning duplicate backend summary jobs and an unbounded toast spam.
  const autoGeneratedForMeetingRef = useRef<string | null>(null);
  const summaryGenerationRef = useRef(summaryGeneration);
  ...
  useEffect(() => {
    if (
      ...
      autoGeneratedForMeetingRef.current === meeting.id
    ) {
      return;
    }
    ...
    (async () => {
      await summaryGenerationRef.current.handleGenerateSummary("");
      if (!cancelled) {
