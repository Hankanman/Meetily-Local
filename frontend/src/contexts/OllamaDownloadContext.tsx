...
const OllamaDownloadContext = createContext<OllamaDownloadContextType | null>(
  null,
);
...
export const useOllamaDownload = () => {
  ...
  if (!context) {
    throw new Error(
      "useOllamaDownload must be used within an OllamaDownloadProvider",
    );
  }
  return context;
};

export function OllamaDownloadProvider({
  children,
}: {
  ...
  useEffect(() => {
    ...
    const setupListeners = async () => {
      try {
        ...
        console.log(
          "[OllamaDownloadContext] Event listeners set up successfully",
        );
      } catch (error) {
        console.error(
          "[OllamaDownloadContext] Failed to set up event listeners:",
          error,
        );
      }
    };
