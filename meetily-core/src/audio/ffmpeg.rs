...
const FFMPEG_PATH_ENV_VAR: &str = "MEETILY_FFMPEG_PATH";
...
/// (`~/.local/share/com.meetily.ai/` on Linux, matching the
/// `com.meetily.ai` Tauri identifier) rather than ffmpeg-sidecar's
/// default of "next to the executable" — that default is read-only
/// inside an AppImage's squashfs mount, so a download would succeed but
/// unpacking into it would fail.
...
mod tests {
    ...
    #[test]
    fn ffmpeg_install_dir_uses_app_identifier() {
        let dir = ffmpeg_install_dir();
        assert!(dir.ends_with("com.meetily.ai/ffmpeg"));
    }
}
